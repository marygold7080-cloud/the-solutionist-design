...
/** Returns transform metadata only for canonical public Wix/Base44 image URLs. */
...
const clampDim = (n) =>
  Math.min(Math.max(Math.round(Number(n) || 1), 1), MAX_DIMENSION)

const clamp01 = (n) =>
  Math.min(1, Math.max(0, Number(n) || 0))

export function buildTransformUrl(
  { baseUrl, filename },
  { width, height, crop, focalPoint, quality = 80 }
) {
  const safeWidth = clampDim(width)
  const safeHeight = clampDim(height || width)
  ...
  return `${baseUrl}/v1/${crop ? "fill" : "fit"}/${params.join(
    ","
  )}/${outputName}`
}

export function buildSrcSet(parsed, options) {
  const width = Number(options?.width) || DEFAULT_TRANSFORM_WIDTH

  return DEVICE_PIXEL_RATIOS.map((dpr) => {
    const transformedUrl = buildTransformUrl(parsed, {
      ...options,
      width: width * dpr,
...
/**
 * Returns an optimized image URL when the source is a supported
 * Wix/Base44 public image URL. Unsupported URLs are returned unchanged.
 */
